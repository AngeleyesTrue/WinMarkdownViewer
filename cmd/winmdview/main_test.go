...
// --- Phase 5: calculateCascadePosition 테스트 ---

// TestCalculateCascadePosition_첫번째윈도우 첫 번째 윈도우는 (0, 0)을 반환해야 한다.
func TestCalculateCascadePosition_첫번째윈도우(t *testing.T) {
	t.Parallel()

	x, y := calculateCascadePosition(0)
	if x != 0 || y != 0 {
		t.Errorf("첫 번째 윈도우 위치: want (0, 0), got (%d, %d)", x, y)
	}
}

// TestCalculateCascadePosition_두번째윈도우 두 번째 윈도우는 (+30, +30) 오프셋을 반환해야 한다.
func TestCalculateCascadePosition_두번째윈도우(t *testing.T) {
	t.Parallel()

	x, y := calculateCascadePosition(1)
	if x != cascadeOffset || y != cascadeOffset {
		t.Errorf("두 번째 윈도우 위치: want (%d, %d), got (%d, %d)",
			cascadeOffset, cascadeOffset, x, y)
	}
}

// TestCalculateCascadePosition_세번째윈도우 세 번째 윈도우는 (+60, +60) 오프셋을 반환해야 한다.
func TestCalculateCascadePosition_세번째윈도우(t *testing.T) {
	t.Parallel()

	x, y := calculateCascadePosition(2)
	if x != 2*cascadeOffset || y != 2*cascadeOffset {
		t.Errorf("세 번째 윈도우 위치: want (%d, %d), got (%d, %d)",
			2*cascadeOffset, 2*cascadeOffset, x, y)
	}
}

// TestCalculateCascadePosition_음수인덱스 음수 인덱스는 (0, 0)을 반환해야 한다.
func TestCalculateCascadePosition_음수인덱스(t *testing.T) {
	t.Parallel()

	x, y := calculateCascadePosition(-1)
	if x != 0 || y != 0 {
		t.Errorf("음수 인덱스 위치: want (0, 0), got (%d, %d)", x, y)
	}
}
